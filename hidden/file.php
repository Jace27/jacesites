...
if (isset($_POST['password'])){
    ...
    while($row = $result->fetch_assoc()){
        if (password_verify($_POST['password'], $row['password'])){
            $file = $mysqli->query("SELECT `file`, `downloadable`, `mime` FROM `hidden` WHERE `id`=".$row['id']);
            if ($file){
                $file = $file->fetch_assoc();
                $mysqli->query("DELETE FROM `hidden` WHERE `id`=".$row['id']);
                header('Content-Type: '.$file['mime']);
                echo $file['downloadable'].'|'.base64_decode(encrypt_decrypt('decrypt', file_get_contents(__DIR__.'/uploaded/'.$file['file']), $_POST['password']));
